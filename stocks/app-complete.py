...
df1['bear_bull'] = (df1['Adj. Close'].diff(-1) < 0).astype(int)
...
# 30 days out
forecast_out = int(math.ceil( 0.010 * len(df1) ))
...
predictedClose = np.array(clf.predict(X_lately))

df = pd.DataFrame(data = predictedClose, columns = ['Predicted Close'])
df['bear_bull'] = (df['Predicted Close'].diff(-1) < 0).astype(int)
df['ROI (%)'] = ((df['Predicted Close'].shift(-1) - df['Predicted Close']) / df['Predicted Close']) * 100
df['Buys'] = (df['Predicted Close']

CumulativeROI = df['ROI (%)'].sum()

print(df.head(30))
print('Cumulative ROI over 30 days: ', CumulativeROI)

## Uses the prediction over 30 days to report whether to buy, sell or hold the stock
#def buySellHold():
#    tomorrowPrice = predictedClose[0]
#    priceInThirtyDays = predictedClose[-1]
#
